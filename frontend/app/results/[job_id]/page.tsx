...
export default function ResultsDashboard({ params }: { params: { job_id: string } }) {
  ...
  if (error && !results) {
    return (
      <div className="flex min-h-[60vh] flex-col items-center justify-center rounded-[32px] border border-cyan-400/10 bg-cyan-400/5 p-12 text-center">
        <div className="mb-3 text-sm font-semibold uppercase tracking-widest text-cyan-300/60">
          Workspace not ready
        </div>
        ...
        <div className="max-w-none text-lg">
          <p className="min-h-[60px] leading-relaxed text-cyan-50/80">
            {explanation
              ...
              : aiError
              ? <span className="text-rose-300 font-medium">⚠ AI analysis failed: {aiError}</span>
